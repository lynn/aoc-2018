...
func main() {
  ...
  for {
    for _, x := range problem_input {
      if (seen[acc]) { fmt.Println(acc); return; }
      seen[acc] = true
      acc += x
    }
  }
